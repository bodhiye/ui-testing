...
| 用例ID | 功能模块 | 优先级 | 用例名称 | 类型 | 执行状态 | 备注 |
|--------|---------|--------|---------|------|---------|------|
| 001 | 代码保存 | P0 | 纯文本代码保存完整流程测试 | E2E | 成功 | 生成分享链接并可访问 |
| 002 | 代码保存 | P0 | 带密码保护的代码保存流程测试 | E2E | 成功 | 出现密码提示，输入后可查看 |
| 003 | 代码保存 | P0 | 阅后即焚功能测试 | E2E | 成功 | 二次访问进入 `/not_found` |
| 004 | 代码保存 | P1 | 不同语言类型保存测试 | E2E | 成功 | Bash / C++ / Java / JSON / Markdown 均通过 |
| 005 | 代码保存 | P1 | 不同过期时间设置测试 | E2E | 成功 | `none/hour/day/week/month/year` 均可创建并验证访问 |
| 006 | 页面导航 | P2 | 页面导航和链接测试 | 展示 | 成功 | GitHub 与备案链接可打开；博客链接连接被拒绝；`mailto:` 已校验 |
